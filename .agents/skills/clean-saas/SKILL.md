...
- Orderful clean logistics SaaS: canvas `#f5f5f5/#ffffff`, ink `#000000/#1f1f1f`, muted text `#4a5565`, border `#d4d4d4`, action `#e42b0c`, CTA gradient `linear-gradient(94deg, rgb(255,2,1) -33.85%, rgb(255,120,2) -1.54%, rgb(255,2,1) 73.55%)`; Telegraf 100-700 with 48/60/72px display at `1` leading and up to `-2.16px` tracking; ModernGothic 14px/1.43 labels; 1440px max, 8px base, 40px sections, 16px card padding/gap, 8px cards, 4px inputs, shadow `rgba(0,0,0,.1) 0 1px 3px, rgba(0,0,0,.1) 0 1px 2px -1px`.
- Ventriloc operations SaaS: `#202020/#ffffff/#efefef/#f5f5f5/#ebe6dd/#4d4d4d/#828282`, signal `#ff682c`, data `#816729`; Inter 400/500/600 12-18px plus PolySans 400 12-66px, display `66px/.91` with `-0.0200em`; 4px base, 80px sections, 40px card padding, 20px gaps; cards 8px, buttons/inputs 20px, avatars 200px; nav hover is a 2px bottom border, not a color wash.
- Tailscale secure network SaaS: `#ffffff/#f7f5f4/#eeebea/#181717/#2e2d2d/#575555/#d5d3d2`, action `#d04841`; Inter 300-600 12-64px, line-height `1.20/1.43/1.50`, tracking `-0.36px` to `-1.92px`; MDIO mono 500 at 12/14/20px with `0.516/0.602/0.86px`; 4px base, 24-64px sections, 24px card padding, 12px gaps; buttons 8px, cards 16px, large panels 32px, pills 9999px, shadow `rgba(24,23,23,.02) 0 4px 8px`.
...
Refero ready-to-use deltas from Orderful/Ventriloc/Tailscale:
- Token roles: `--saas-canvas:#f5f5f5`, `--saas-surface:#ffffff`, `--saas-ink:#181717`, `--saas-muted:#4a5565`, `--saas-line:#d4d4d4`, `--saas-action:#e42b0c`, `--saas-action-gradient:linear-gradient(94deg, rgb(255,2,1) -33.85%, rgb(255,120,2) -1.54%, rgb(255,2,1) 73.55%)`; operational dashboards can add `--saas-signal:#ff682c` and `--saas-data:#816729`.
- Shadow/radius contract: default SaaS card `8px` plus `rgba(0,0,0,.1) 0 1px 3px, rgba(0,0,0,.1) 0 1px 2px -1px`; secure/network panels use `16px/32px` and `rgba(24,23,23,.02) 0 4px 8px`.
- Refero typography delta: Orderful uses Telegraf 100-700 display at 48/60/72px with `1` leading and up to `-2.16px`; Ventriloc uses PolySans 12-66px at `.91` display leading; Tailscale uses Inter 300-600 plus MDIO mono 12/14/20px with `0.516/0.602/0.86px` tracking.
...
- Orderful-style SaaS: 1440px max, 8px base grid, 40px section rhythm, 16px card padding/gap; first viewport shows workflow/logistics proof beside a CTA.
- Ventriloc-style ops: 80px section gaps, 40px card padding, 20px gaps, 8px cards, 20px buttons/inputs.
- Tailscale-style secure network: 24-64px section rhythm, 24px card padding, 12px gaps, 8px buttons, 16px cards, 32px major panels, 9999px pills.
...
- Primary CTA families: Orderful orange/gradient CTA with 8px radius and 12px 24px padding; Ventriloc pill/soft control at 20px radius; Tailscale secure CTA at 8px radius with `#d04841`.
- Inputs: Orderful uses 4px radius with 16px padding; Ventriloc uses 20px rounded fields; Tailscale uses 9999px pills only for tags/status.
- Proof cards: Orderful metric cards use the two-layer 1-3px shadow; Ventriloc cards stay flat `#efefef`; Tailscale panels use low-alpha shadow and surface steps.
...
Refero state deltas:
- Hover: Ventriloc nav hover is a 2px bottom border; do not replace it with a tinted background.
- Focus: Orderful inputs use a 4px geometry with `#e42b0c` or `#d4d4d4` ring; Tailscale secure controls use `#d04841` only for primary action/focus.
- Status: use `#ff682c` for Ventriloc signal, `#816729` for data, and Tailscale muted grays `#575555/#d5d3d2` for disabled/info states.
...
Refero motion delta:
- No source-specific easing was observed for Orderful, Ventriloc, or Tailscale. Use existing skill timing; limit motion to opacity/translate/card hover, and never animate table row height, form layout, or dashboard metrics.
...
- Refero anti-dilution: do not combine Orderful orange gradient, Ventriloc `#ff682c`, and Tailscale `#d04841` as three competing CTAs; choose one action system per page.
- Refero anti-radius drift: do not apply 9999px pills to every component; Tailscale pills are tags/status, Ventriloc rounds controls at 20px, Orderful inputs are 4px.
- Refero anti-generic: do not use generic purple SaaS accents unless the chosen source pack already contains them.
