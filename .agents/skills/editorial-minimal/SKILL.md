...
- ElevenLabs blueprint editorial: `#fdfcfc/#f5f3f1/#e5e5e5/#b1b0b0/#777169/#a59f97/#000000`, micro accents `#0447ff/#ff4704`, voice mark `conic-gradient(from 180deg, rgb(61,117,216) 11.5931deg, rgb(82,208,233) 38.9146deg, rgb(30,83,176) 343.551deg)`; Waldenburg 300 at 32/36/48px with 1.08-1.17 leading and `-0.64px` to `-0.96px`; Inter 10-20px with 1.0-2.06 leading; 1200px max, 80-120px sections, 16-24px cards, 8-12px gaps, pills 9999px, panels 20px, inputs 4px.
- Granola warm note system: near-white canvas with olive accents `#b2c248/#5b6f00/#788c15`, dark ink and mid-gray text; Quadrant serif headlines paired with Melange mono-inflected UI; use 0.5px hairline borders, white note cards, muted-yellow-green CTA fills, and flat ruled card edges. Accent green is for action/status, not a full page tint.
- Slite parchment desk: page `#f9efe4`, cards `#fdfdfd/#fdf9f4`, ink `#3f434a`, CTA `#2d2f34`, link `#2e77e5`, badge pairs such as butter `#fbf4d8` with ochre `#7f6c1f`; Garnett 500/700 at 24/32/36/64px with `ss14/ss15/ss19`; 1200px max, 80-120px sections, 24px grid gutters, buttons 42-50px, cards 12px with three-layer whisper shadow.
- State transfer: ElevenLabs selected row `#f5f3f1`, disabled text `#b1b0b0`, focus ring `1px #e5e5e5`; Granola hover fills use olive `#b2c248` at low alpha; Slite active CTA stays `#2d2f34` with link focus `#2e77e5`.
...
Refero ready-to-use deltas from ElevenLabs/Granola/Slite:
- Token roles: ElevenLabs `#fdfcfc/#f5f3f1/#e5e5e5/#777169/#000000` with micro accents `#0447ff/#ff4704`; Granola olive states `#b2c248/#5b6f00/#788c15`; Slite parchment `#f9efe4/#fdfdfd/#fdf9f4/#3f434a` with CTA `#2d2f34` and link `#2e77e5`.
- Type roles: Waldenburg 300 at 32/36/48px with `-0.64px` to `-0.96px`; Granola uses Quadrant serif + Melange mono-inflected UI; Slite uses Garnett 24/32/36/64px plus UniversalSans 12-16px.
...
- ElevenLabs display: Waldenburg 300, 32/36/48px, 1.08-1.17 leading, `-0.02em` tracking; body/meta Inter 10-20px.
- Slite heading stack: Garnett 500/700 24-64px with `ss14/ss15/ss19`; body UniversalSans 12-16px.
- Do not replace these with generic Inter-only headings when using the new sources.
...
- ElevenLabs: 1200px max, 80-120px sections, 16-24px cards, 8-12px gaps, 20px panels, 4px inputs.
- Granola: white note cards, 0.5px hairlines, flat ruled edges; keep note density readable.
- Slite: 1200px max, 80-120px sections, 24px grid gutters, 12px cards, 42-50px buttons.
...
- ElevenLabs: 9999px pill CTAs/tags, 20px voice/demo panels, 4px transparent inputs.
- Granola: white note cards with olive CTA/status fills; no full-page green tint.
- Slite: dark CTA `#2d2f34`, link `#2e77e5`, 12px cards, 8px tooltip, three-layer whisper shadow.
...
Refero state deltas:
- ElevenLabs selected row `#f5f3f1`, disabled `#b1b0b0`, focus `1px #e5e5e5`.
- Granola hover fills use olive `#b2c248` at low alpha; Slite active CTA stays `#2d2f34` with link focus `#2e77e5`.
...
Refero motion delta:
- No source-specific durations were observed for ElevenLabs/Granola/Slite. Use existing editorial-minimal timing; allow opacity/translate and focus/underline continuity only.
...
- Refero anti-dilution: do not turn ElevenLabs blue/orange dots, Granola olive, and Slite blue into a multi-accent system.
- Do not add decorative shadows beyond Slite whisper shadow or the source-specific subtle inset treatments.
