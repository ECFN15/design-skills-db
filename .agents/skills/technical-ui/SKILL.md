...
- Authkit authentication console: `#05060f/#ffffff/#d8ecf8/#d1e4fa/#b6d9fc/#81899b/#9da7ba`, action `#663af3`, divider `linear-gradient(90deg, rgba(0,0,0,0), rgba(186,215,247,.12), rgba(0,0,0,0))`; Untitled Sans 400-700 12-24px, aeonikPro 28/44/48px, dotDigital 15px/1.2 with `0.1em` and tabular numerals; 4px base, 48px sections, 24px cards, 8px gaps; inputs 2-4px, badges 6px, cards 12-16px, buttons 999px; glass cards use inset `rgba(199,211,234,.12)` and deep `rgba(6,6,14,.7)`.
- Inngest event workflow UI: `#0c0a09/#1c1917/#44403c/#ffffff/#f6f6f6/#a89984`, accent `#cab16a`, success `#59a569`, warning `#cc5b33`, error `#ea6962`; CircularXX 300-600 12-72px with `-0.019em`, CircularXXMono 12/14/18px, Whyte Inktrap 72px `-0.056em`; 8px base, 24-40px sections, 4-24px gaps, 0px card padding, 4px controls, 9999px pills.
- Depot build dashboard: `#04040b/#121113/#323035/#3c393f/#eeeef0/#b5b2bc`, success action `#71d083`, action bg `#366740`, link `#70b8ff`, error `#ff9592`; Red Hat Display 400-700 14-60px `-0.0250em`, Red Hat Text 10-20px `0.0250em`, Red Hat Mono 14/18px; 8px base, 64px sections, 16px card padding, 8px gap; 2px small controls, 6px cards/buttons, 10px key interactives; no decorative glow.
...
Refero ready-to-use deltas from Authkit/Inngest/Depot:
- Token roles: Authkit action `#663af3` with pale blue `#d8ecf8/#d1e4fa/#b6d9fc` and divider `linear-gradient(90deg, rgba(0,0,0,0), rgba(186,215,247,.12), rgba(0,0,0,0))`; Inngest success/warn/error `#59a569/#cc5b33/#ea6962`; Depot success/link/error `#71d083/#70b8ff/#ff9592`.
- Type roles: Authkit uses Untitled Sans 12-24px, aeonikPro 28/44/48px, dotDigital 15px tabular codes; Inngest uses CircularXX 12-72px plus mono 12/14/18px; Depot uses Red Hat Display/Text/Mono with explicit tracking.
...
- Authkit: 4px base, 48px sections, 24px cards, 8px gaps; show authentication card plus verification/status rail.
- Inngest: 8px base, 24-40px sections, 4-24px gaps, 0px card padding for dense event lists.
- Depot: 8px base, 64px sections, 16px card padding, 8px gaps; expose build/log/status evidence.
...
- Authkit: 2-4px inputs, 6px badges, 12-16px glass cards, 999px buttons, dotDigital OTP/session numerals.
- Inngest: 4px controls, 9999px pills, flat workflow/event rows with CircularXXMono IDs.
- Depot: 2px small controls, 6px cards/buttons, 10px key interactives; success button uses `#366740` bg with `#71d083` text.
...
Refero state deltas:
- Authkit focus/action is `#663af3`; divider/hover can use the pale-blue gradient, not a new glow.
- Inngest success/warn/error: `#59a569/#cc5b33/#ea6962`; Depot success/link/error: `#71d083/#70b8ff/#ff9592`.
...
Refero motion delta:
- No source-specific duration was observed for Authkit, Inngest, or Depot. Use existing technical-ui timing; animate opacity, border, and transform only, never log row heights or live metric positions.
...
- Refero anti-dilution: Authkit is the only glass/inset-shadow branch; do not add glow to Depot or Inngest panels.
- Do not average radius systems: Authkit 2/4/6/12/16/999, Inngest 4/9999, Depot 2/6/10.
