...
- 099 monochrome utility index: `#000000/#1d1d1d/#383838/#888888/#ffffff`; Soehne Mono 400 only at 16px with `1/1.2/1.4` leading, `0.24px` tracking, slashed zero; 4px base, 1600px max, 48px sections, 16px gaps, 26.5px info padding; default 0px, cards/inputs/buttons 10px; ghost controls use `1px #383838`, padding `19.2px 25.6px`, selected controls fill `#1d1d1d`.
- Slash black editorial utility: `#000000/#030304/#08080a/#121317/#1c1d22`, text `#ffffff/#e2e3e9/#cdcdcd/#777a88/#5e616e`, one golden gradient for selected action only; Inter 300-700 12-52px with `-0.007em` to `-0.04em`, Ivy Presto 400/500 32/52/64/88px; 1216px max, 160px sections, 32px cards, 6px element gaps; 2px small controls, 10px cards, 9999px pills.
- Silencio archival utility: `#ffffff/#000000/#dbdad9/#808080`, background gradient `linear-gradient(0deg, rgb(219,218,217), rgb(255,255,255))`; HaasR 100/400/700 at 12/16/19/22/39/58px, HaasT 141px, PT Mono 11px/1.2; 4px base, 29px card padding, 6-14px element rhythm; 7.2px cards, 43.2px large, 129.6px pill buttons; no chroma or shadow.
...
Refero ready-to-use deltas from 099/Slash/Silencio:
- Token roles: 099 strict utility `#000000/#1d1d1d/#383838/#888888/#ffffff`; Slash black ledger `#030304/#08080a/#121317/#1c1d22` with text `#ffffff/#e2e3e9/#cdcdcd/#777a88/#5e616e`; Silencio archival `#ffffff/#000000/#dbdad9/#808080` plus `linear-gradient(0deg, rgb(219,218,217), rgb(255,255,255))`.
- Type roles: 099 Soehne Mono 16px with `0.24px`; Slash Inter 12-52px plus Ivy Presto 32/52/64/88px; Silencio HaasR 12-58px, HaasT 141px, PT Mono 11px.
...
- 099: 4px base, 1600px max, 48px sections, 16px gaps, 26.5px info padding; index/control surface first.
- Slash: 1216px max, 160px sections, 32px cards, 6px element gaps; black product ledger.
- Silencio: 4px base, 29px cards, 6-14px rhythm, 7.2px cards and 129.6px pill buttons; archival density.
...
- 099: ghost controls `1px #383838`, padding `19.2px 25.6px`, selected fill `#1d1d1d`, search input `6.4px 19.2px`, controls/cards 10px.
- Slash: 2px small controls, 10px cards, 9999px pills, off-white interaction text; golden gradient only for selected state.
- Silencio: 7.2px cards, 43.2px large modules, 129.6px buttons, PT Mono 11px metadata.
...
Refero state deltas:
- 099 selected state fills `#1d1d1d`; hover/focus keeps `1px #383838` outline.
- Slash selected/elevated action may use one golden gradient; disabled text uses `#5e616e/#777a88`.
- Silencio hover inversion uses transparent/white surface with black border; no chroma.
...
Refero motion delta:
- No source-specific durations were observed for 099, Slash, or Silencio. Use existing utilitarian timings; prefer instant border/fill changes and avoid decorative reveals.
...
- Refero anti-dilution: do not add blue/green/red status colors to 099 or Silencio unless a real labeled state requires them.
- Do not add soft shadows to 099; do not add chroma to Silencio; do not use Slash gold as background decoration.
