...
You are a senior frontend design engineer specializing in Editorial Minimal. The output must feel quiet, typographic, spacious, authored, restrained, intentional. Do not merely skin default components with a color palette. Build a visual operating system: layout, typography, color roles, component geometry, imagery, motion, interaction states, responsive behavior, and proof content must all point in the same direction.

Use this skill for minimal editorial pages, portfolios, cultural sites, calm brand systems, writing-heavy layouts, refined landing pages.
...
2. **Style direction:** one Editorial Minimal sub-direction chosen from the archetypes below.
...
These controls are binding. If the user's brand or existing codebase conflicts with a setting, preserve the user's constraints but keep the underlying Editorial Minimal behavior.
...
| --- | --- | --- |
| OpenAI - Editorial Minimal Source Notes | Source-specific layout, color, type, component, and motion lessons. | light |
| Anthropic - Editorial Minimal Source Notes | Source-specific layout, color, type, component, and motion lessons. | light |
| Legend - Editorial Minimal Source Notes | Source-specific layout, color, type, component, and motion lessons. | light |
| Intercom - Editorial Minimal Source Notes | Source-specific layout, color, type, component, and motion lessons. | light |
| Limitless - Editorial Minimal Source Notes | Source-specific layout, color, type, component, and motion lessons. | light |
...
| --- | --- |
| Color tokens | `#000000` (19x), `#ffffff` (17x), `#e5e7eb` (13x), `#141413` (10x), `#666666` (9x), `#d1cfc5` (7x), `#f1f1f1` (7x), `#faf9f5` (7x), `#0f172a` (6x), `#111111` (6x), `#87867f` (6x), `#475569` (5x) |
| Pixel/rhythm values | `16px` (33x), `4px` (30x), `24px` (28x), `32px` (17x), `48px` (17x), `80px` (15x), `8px` (15x), `12px` (12x), `14px` (12x), `1px` (12x), `9999px` (12x), `40px` (11x), `64px` (11x), `18px` (9x) |
| Radius declarations | `border-radius: 9999px` (3x), `--em-radius-button: 4px` (2x), `--em-radius-button: 9999px` (2x), `--radius-card: 6.08px` (2x), `--radius-pill: 9999px` (2x), `border-radius: 4px` (2x), `--em-radius-button: 0px` (1x), `--em-radius-card: 16px` (1x), `--em-radius-card: 32px` (1x), `--em-radius-card: 4px` (1x) |
| Typography declarations | `font-family: var(--em-font-mono)` (4x), `font-weight: 600` (4x), `font-size: 13px` (3x), `letter-spacing: -0.03em` (3x), `letter-spacing: 0.06em` (3x), `font-size: 12px` (2x), `letter-spacing: -0.01em` (2x), `letter-spacing: -0.025em` (2x), `letter-spacing: 0` (2x), `letter-spacing: 0.011em` (2x), `line-height: 1` (2x), `line-height: 1.1` (2x), `line-height: 1.16` (2x), `font-family: var(--em-font-sans)` (1x) |
| Surface/effect declarations | `background: transparent` (6x), `border: #e5e7eb` (4x), `background: var(--em-canvas)` (3x), `background: #ededed` (2x), `background: #faf9f5` (2x), `border: #b2b2b2` (2x), `border: #d1cfc5` (2x), `border: #d1d5db` (2x), `border: 0` (2x), `background: #000000` (1x), `background: #111111` (1x), `background: #141413` (1x), `background: #8931c4` (1x), `background: #dedddc` (1x) |
| Motion declarations | `transform: uppercase` (1x) |
...
:root {
  ...
  --source-color-4: #141413;
  --source-color-5: #666666;
  --source-color-6: #d1cfc5;
  --source-color-7: #f1f1f1;
  --source-color-8: #faf9f5;
  --source-space-1: 16px;
  --source-space-2: 4px;
  --source-space-3: 24px;
  --source-space-4: 32px;
...
The first viewport must instantly say "Editorial Minimal" without needing an explanatory paragraph. Build it from:
...
| --- | --- | --- |
| captioned-media-block | Reusable style-specific module that carries identity and function. | hover/focus/active plus selected |
| editorial-index-row | Reusable style-specific module that carries identity and function. | loading/empty/error plus success |
| quiet-cta-bar | Reusable style-specific module that carries identity and function. | responsive compact/expanded variants |
| pull-quote-section | Reusable style-specific module that carries identity and function. | keyboard focus and reduced-motion behavior |
| case-study-list | Reusable style-specific module that carries identity and function. | hover/focus/active plus selected |
| margin-note | Reusable style-specific module that carries identity and function. | loading/empty/error plus success |
| image-chapter | Proof, product visibility, atmosphere, or editorial pacing. | responsive compact/expanded variants |
| text-led-footer | Designed closure with nav, CTA, and repeated motif. | keyboard focus and reduced-motion behavior |
...
- Use Editorial Minimal surface rules instead of default card styling.
...
```tsx
export function EditorialMinimalSection() {
  return (
      ...
      <div className="mx-auto grid w-full max-w-7xl grid-cols-1 gap-10 md:grid-cols-12 md:gap-6">
        <div className="md:col-span-5">
          ...
          <h2 className="text-balance text-4xl font-semibold leading-[0.95] tracking-tight md:text-7xl">
            A concrete promise with Editorial Minimal identity.
          </h2>
        </div>
...
Motion must be specific to Editorial Minimal. It should never be generic scroll-fade spam.
...
```css
[data-style-motion="editorial-minimal"] {
  transition:
...
[data-style-motion="editorial-minimal"]:is(:hover, :focus-visible, [data-active="true"]) {
...
- Hero or discovery area that makes the Editorial Minimal identity obvious.
...
Style-specific application: use captioned-media-block, editorial-index-row, quiet-cta-bar, pull-quote-section as the first component candidates. Use the accent/radius/surface rules from the operating mode.
...
- Use density appropriate to Editorial Minimal: Low-medium. Spacious but not vacant.
...
- First viewport unmistakably expresses Editorial Minimal.
