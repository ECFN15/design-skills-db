...
description: "Use this skill to create Expressive Brand visual design systems that feel distinctive, memorable, colorful, confident, personality-led, specific. USE FOR: brand-heavy sites, consumer products, creative launches, expressive agencies, campaigns, identity systems. DO NOT USE FOR: unrelated backend work, non-visual tasks, or when an existing product design system must be followed exactly."
---

# Expressive Brand Skill

## Core Directive

You are a senior frontend design engineer specializing in Expressive Brand. The output must feel distinctive, memorable, colorful, confident, personality-led, specific. Do not merely skin default components with a color palette. Build a visual operating system: layout, typography, color roles, component geometry, imagery, motion, interaction states, responsive behavior, and proof content must all point in the same direction.

Use this skill for brand-heavy sites, consumer products, creative launches, expressive agencies, campaigns, identity systems.
...
2. **Style direction:** one Expressive Brand sub-direction chosen from the archetypes below.
...
These controls are binding. If the user's brand or existing codebase conflicts with a setting, preserve the user's constraints but keep the underlying Expressive Brand behavior.
...
| --- | --- | --- |
| Family - Expressive Brand Source Notes | Source-specific layout, color, type, component, and motion lessons. | light |
| mymind - Expressive Brand Source Notes | Source-specific layout, color, type, component, and motion lessons. | light |
| Antimetal - Expressive Brand Source Notes | Source-specific layout, color, type, component, and motion lessons. | mixed |
| Empower - Expressive Brand Source Notes | Source-specific layout, color, type, component, and motion lessons. | mixed |
| Retool - Expressive Brand Source Notes | Source-specific layout, color, type, component, and motion lessons. | dark |
...
| --- | --- |
| Color tokens | `#ffffff` (21x), `#e9ebdf` (12x), `#151515` (10x), `#ff5924` (9x), `#121212` (7x), `#1b2540` (7x), `#f2f0ed` (7x), `#fbfaf9` (7x), `#ffbb26` (7x), `#100f0f` (6x), `#242424` (6x), `#3f403d` (6x) |
| Pixel/rhythm values | `24px` (16x), `32px` (13x), `8px` (12x), `16px` (10x), `1px` (9x), `72px` (9x), `20px` (8x), `4px` (7x), `9999px` (7x), `10px` (6x), `1200px` (6x), `12px` (6x), `44px` (5x), `18px` (4x) |
| Radius declarations | `border-radius: 9999px` (5x), `border-radius: 16px` (2x), `border-radius: 20px` (2x), `border-radius: 8px` (2x), `--radius-brand-card: 10px` (1x), `--radius-brand-character: 72px` (1x), `--radius-brand-large: 24px` (1x), `--radius-brand-pill: 32px` (1x), `--radius-card: 10px` (1x), `--radius-card: 24px` (1x) |
| Typography declarations | `font-weight: 600` (3x), `text-transform: uppercase` (3x), `font-size: 13px` (2x), `font-weight: 700` (2x), `letter-spacing: .09em` (2x), `font-family: var(--font-display)` (1x), `font-family: var(--font-label)` (1x), `font-family: var(--font-sans)` (1x), `font-size: 12px` (1x), `font-size: 16px` (1x), `font-size: 17px` (1x), `font-size: clamp(44px, 7vw, 88px)` (1x), `font-weight: 500` (1x), `font-weight: 650` (1x) |
| Surface/effect declarations | `background: #ffffff` (3x), `background: #242424` (2x), `background: #ff5924` (2x), `border: 1px solid #3f403d` (2x), `box-shadow: 0 0 0 1px rgba(0,39,80,.04),` (2x), `background: #121212` (1x), `background: #151515` (1x), `background: #262525` (1x), `background: #d0f100` (1x), `background: #e4e24e` (1x), `background: #e9ebdf` (1x), `background: #f8f7f4` (1x), `background: #fbfaf9` (1x), `background: radial-gradient(circle at 50% 30%, rgba(0,144,255,.34), transparent 38%),` (1x) |
| Motion declarations | `transform: uppercase` (3x), `@keyframes character-float` (1x), `@keyframes shimmer` (1x), `@keyframes signal-reveal` (1x), `animation: character-float 4.8s ease-in-out infinite` (1x), `animation: shimmer 2.8s linear infinite` (1x), `animation: signal-reveal 620ms var(--ease-signal) forwards` (1x), `transform: translateY(-8px) rotate(1deg)` (1x), `transform: translateY(0) rotate(-1deg)` (1x), `transform: translateY(0) scale(1)` (1x), `transform: translateY(18px) scale(.98)` (1x) |
...
:root {
  ...
  --source-color-2: #e9ebdf;
  --source-color-3: #151515;
  --source-color-4: #ff5924;
  --source-color-5: #121212;
  --source-color-6: #1b2540;
  --source-color-7: #f2f0ed;
  --source-color-8: #fbfaf9;
  --source-space-1: 24px;
  --source-space-2: 32px;
  --source-space-3: 8px;
  --source-space-4: 16px;
...
The first viewport must instantly say "Expressive Brand" without needing an explanatory paragraph. Build it from:
...
| --- | --- | --- |
| brand-motif-hero | First-viewport identity and proof object. | hover/focus/active plus selected |
| voice-driven-cta | Reusable style-specific module that carries identity and function. | loading/empty/error plus success |
| color-coded-card-system | Reusable style-specific module that carries identity and function. | responsive compact/expanded variants |
| campaign-strip | Reusable style-specific module that carries identity and function. | keyboard focus and reduced-motion behavior |
| object-gallery | Proof, product visibility, atmosphere, or editorial pacing. | hover/focus/active plus selected |
| animated-logo-grid | Structured animation or sequence. | loading/empty/error plus success |
| persona-testimonial | Reusable style-specific module that carries identity and function. | responsive compact/expanded variants |
| expressive-footer | Designed closure with nav, CTA, and repeated motif. | keyboard focus and reduced-motion behavior |
...
- Use Expressive Brand surface rules instead of default card styling.
...
```tsx
export function ExpressiveBrandSection() {
  return (
      ...
      <div className="mx-auto grid w-full max-w-7xl grid-cols-1 gap-10 md:grid-cols-12 md:gap-6">
        <div className="md:col-span-5">
          ...
          <h2 className="text-balance text-4xl font-semibold leading-[0.95] tracking-tight md:text-7xl">
            A concrete promise with Expressive Brand identity.
          </h2>
        </div>
...
Motion must be specific to Expressive Brand. It should never be generic scroll-fade spam.
...
[data-style-motion="brand-motif"]:is(:hover, :focus-visible, [data-active="true"]) {
  transform: translateY(-3px) rotate(var(--brand-tilt, -1deg)); filter: saturate(1.06);
}
...
- Hero or discovery area that makes the Expressive Brand identity obvious.
...
Style-specific application: use brand-motif-hero, voice-driven-cta, color-coded-card-system, campaign-strip as the first component candidates. Use the accent/radius/surface rules from the operating mode.
...
- Use density appropriate to Expressive Brand: Medium. Let brand moments breathe, then support with clear product sections.
...
- First viewport unmistakably expresses Expressive Brand.
