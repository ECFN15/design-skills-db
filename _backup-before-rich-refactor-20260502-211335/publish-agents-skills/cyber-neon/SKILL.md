...
You are a senior frontend design engineer specializing in Cyber Neon. The output must feel electric, nocturnal, luminous, futuristic, sharp, high-energy. Do not merely skin default components with a color palette. Build a visual operating system: layout, typography, color roles, component geometry, imagery, motion, interaction states, responsive behavior, and proof content must all point in the same direction.

Use this skill for futuristic websites, cyberpunk launches, gaming, AI labs, nightlife brands, luminous dark interfaces, techno editorial pages.
...
2. **Style direction:** one Cyber Neon sub-direction chosen from the archetypes below.
...
These controls are binding. If the user's brand or existing codebase conflicts with a setting, preserve the user's constraints but keep the underlying Cyber Neon behavior.
...
| --- | --- | --- |
| Chainzoku - Style Reference | Source-specific layout, color, type, component, and motion lessons. | Extract transferable visual rules, not a clone. |
| SBS Town - Style Reference | Source-specific layout, color, type, component, and motion lessons. | Extract transferable visual rules, not a clone. |
| Neon (neon.tech) - Style Reference | Source-specific layout, color, type, component, and motion lessons. | Extract transferable visual rules, not a clone. |
| JetBrains - Style Reference | Source-specific layout, color, type, component, and motion lessons. | Extract transferable visual rules, not a clone. |
| OFF WHITE - Style Reference | Source-specific layout, color, type, component, and motion lessons. | Extract transferable visual rules, not a clone. |
...
| --- | --- |
| Color tokens | `#000000` (27x), `#ffffff` (21x), `#bababb` (17x), `#00fb34` (16x), `#000` (15x), `#34d59a` (15x), `#fff` (13x), `#151617` (12x), `#19191c` (9x), `#303236` (9x), `#cdfb52` (9x), `#0a0a0b` (8x) |
| Pixel/rhythm values | `4px` (53x), `16px` (47x), `24px` (39x), `1px` (33x), `9999px` (27x), `100px` (26x), `12px` (22x), `0px` (21x), `32px` (20x), `8px` (18x), `30px` (16x), `20px` (15x), `2px` (13x), `6px` (13x) |
| Radius declarations | `border-radius: 24px` (6x), `border-radius: 4px` (5x), `border-radius: 9999px` (5x), `--radius-buttons: 9999px` (4x), `--radius-cards: 4px` (4x), `border-radius: 16px` (4x), `--radius-badges: 4px` (3x), `border-radius: 0` (3x), `border-radius: var(--radius-buttons)` (3x), `border-radius: var(--radius-cards)` (3x) |
| Typography declarations | `font-weight: 500` (10x), `font-family: var(--font-display)` (6x), `font-size: 16px` (6x), `font-family: inherit` (5x), `font-size: 13px` (5x), `font-family: var(--cn-font-display)` (4x), `font-weight: 600` (4x), `font-weight: 700` (4x), `line-height: 1.0` (4x), `font-family: var(--font-body)` (3x), `font-family: var(--font-mono)` (3x), `text-transform: uppercase` (3x), `font-family: 'JetBrains Sans', 'Inter', sans-serif` (2x), `font-size: 12px` (2x) |
| Surface/effect declarations | `border: none` (12x), `background: transparent` (7x), `background: #000` (6x), `background: rgba(90, 31, 208, 0.3)` (6x), `background: radial-gradient(` (4x), `background: var(--cn-accent)` (4x), `background: var(--surface-2)` (4x), `background: the dominant page or app surface` (3x), `background: var(--bg)` (3x), `border: 1px solid #5a1fd0` (3x), `border: 1px solid var(--surface-4)` (3x), `border: visible structure and grouping` (3x), `backdrop-filter: blur(12px)` (2x), `background: #000000` (2x) |
| Motion declarations | `duration: 0.01ms !important;` (4x), `transition: filter 200ms ease` (4x), `transform: uppercase` (3x), `@keyframes glitch-shift` (2x), `animation: glow-pulse 8s ease-in-out infinite` (2x), `@keyframes glow-pulse` (1x), `@keyframes scanline-roll` (1x), `animation: glitch-shift 3s steps(3) infinite` (1x), `animation: glitch-shift 4s steps(2) infinite` (1x), `animation: none` (1x), `animation: scanline-roll 8s linear infinite` (1x), `transform: scale(1.05)` (1x), `transform: scale(1)` (1x), `transform: translate(-10%, 10%)` (1x) |
...
:root {
  ...
  --source-color-3: #bababb;
  --source-color-4: #00fb34;
  --source-color-5: #000;
  --source-color-6: #34d59a;
  --source-color-7: #fff;
  --source-color-8: #151617;
  --source-space-1: 4px;
  --source-space-2: 16px;
  --source-space-3: 24px;
  --source-space-4: 1px;
  --source-space-5: 9999px;
  --source-space-6: 100px;
  --source-space-7: 12px;
  --source-space-8: 0px;
...
The first viewport must instantly say "Cyber Neon" without needing an explanatory paragraph. Build it from:
...
| --- | --- | --- |
| neon-command-bar | Reusable style-specific module that carries identity and function. | hover/focus/active plus selected |
| hologram-product-card | Reusable style-specific module that carries identity and function. | loading/empty/error plus success |
| scanline-hero-core | First-viewport identity and proof object. | responsive compact/expanded variants |
| telemetry-chip-rail | Reusable style-specific module that carries identity and function. | keyboard focus and reduced-motion behavior |
| glitch-title-mask | Reusable style-specific module that carries identity and function. | hover/focus/active plus selected |
| luminous-pricing-deck | Conversion, checkout, plan choice, or purchasing confidence. | loading/empty/error plus success |
| system-status-map | Reusable style-specific module that carries identity and function. | responsive compact/expanded variants |
| cursor-reactive-panel | Reusable style-specific module that carries identity and function. | keyboard focus and reduced-motion behavior |
...
- Use Cyber Neon surface rules instead of default card styling.
...
```tsx
export function CyberNeonSection() {
  return (
      ...
      <div className="mx-auto grid w-full max-w-7xl grid-cols-1 gap-10 md:grid-cols-12 md:gap-6">
        <div className="md:col-span-5">
          ...
          <h2 className="text-balance text-4xl font-semibold leading-[0.95] tracking-tight md:text-7xl">
            A concrete promise with Cyber Neon identity.
          </h2>
        </div>
...
Motion must be specific to Cyber Neon. It should never be generic scroll-fade spam.
...
```css
[data-style-motion="cyber-neon-signal"] {
  transition:
...
[data-style-motion="cyber-neon-signal"]:is(:hover, :focus-visible, [data-active="true"]) {
  filter: brightness(1.18) drop-shadow(0 0 14px var(--neon));
}
...
- Hero or discovery area that makes the Cyber Neon identity obvious.
...
Style-specific application: use neon-command-bar, hologram-product-card, scanline-hero-core, telemetry-chip-rail as the first component candidates. Use the accent/radius/surface rules from the operating mode.
...
- Use density appropriate to Cyber Neon: Medium. Use concentrated telemetry clusters and large negative voids.
...
- First viewport unmistakably expresses Cyber Neon.
