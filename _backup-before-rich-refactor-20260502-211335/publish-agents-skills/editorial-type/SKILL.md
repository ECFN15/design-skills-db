...
You are a senior frontend design engineer specializing in Editorial Type. The output must feel literary, expressive, composed, sharp, cultural, typographic. Do not merely skin default components with a color palette. Build a visual operating system: layout, typography, color roles, component geometry, imagery, motion, interaction states, responsive behavior, and proof content must all point in the same direction.

Use this skill for typographic websites, magazine-like pages, cultural brands, essays, archives, portfolios, editorial commerce.
...
2. **Style direction:** one Editorial Type sub-direction chosen from the archetypes below.
...
These controls are binding. If the user's brand or existing codebase conflicts with a setting, preserve the user's constraints but keep the underlying Editorial Type behavior.
...
| --- | --- | --- |
| Volume - Editorial Type Source Notes | Editorial archive, high contrast. | light |
| Victor Cango - Editorial Type Source Notes | Editorial type on stark canvas | light |
| No Ideas - Editorial Type Source Notes | High-contrast editorial publication | dark |
| Sociotype - Editorial Type Source Notes | Editorial White Canvas | light |
| Christopherdoyle - Editorial Type Source Notes | Editorial Grid, Ink on Paper | light |
...
| --- | --- |
| Color tokens | `#ffffff` (40x), `#000000` (26x), `#0f0f0f` (23x), `#272727` (9x), `#f7f7f7` (9x), `#717171` (7x), `#ff5c26` (6x), `#212529` (5x), `#818181` (5x), `#949494` (5x), `#962921` (5x), `#c52910` (5x) |
| Pixel/rhythm values | `0px` (40x), `16px` (27x), `50px` (26x), `12px` (18x), `14px` (18x), `1px` (17x), `48px` (13x), `251px` (12x), `15px` (10x), `21px` (10x), `18px` (9x), `7.5px` (8x), `10px` (6x), `1400px` (6x) |
| Radius declarations | `--style-radius: 8px` (1x), `border-radius: var(--style-radius)` (1x) |
| Typography declarations | `font-size: 18px` (1x), `font-size: clamp(44px, 7vw, 96px)` (1x), `font-weight: 700` (1x), `line-height: 0.98` (1x), `line-height: 1.5` (1x) |
| Surface/effect declarations | `background: var(--style-accent)` (1x), `background: var(--style-bg)` (1x), `border: #e5e7eb` (1x) |
| Motion declarations | `transition: 240-420ms` (1x) |
...
:root {
  ...
  --source-color-3: #0f0f0f;
  --source-color-4: #272727;
  --source-color-5: #f7f7f7;
  --source-color-6: #717171;
  --source-color-7: #ff5c26;
  --source-color-8: #212529;
  --source-space-1: 0px;
  --source-space-2: 16px;
  --source-space-3: 50px;
  --source-space-4: 12px;
...
The first viewport must instantly say "Editorial Type" without needing an explanatory paragraph. Build it from:
...
| --- | --- | --- |
| poster-title-hero | First-viewport identity and proof object. | hover/focus/active plus selected |
| issue-index-table | Dense comparison, operational data, or scannable records. | loading/empty/error plus success |
| pullquote-panel | Reusable style-specific module that carries identity and function. | responsive compact/expanded variants |
| caption-grid | Reusable style-specific module that carries identity and function. | keyboard focus and reduced-motion behavior |
| article-card-row | Reusable style-specific module that carries identity and function. | hover/focus/active plus selected |
| typographic-filter | Browsing, sorting, narrowing, and marketplace/product discovery. | loading/empty/error plus success |
| spread-gallery | Proof, product visibility, atmosphere, or editorial pacing. | responsive compact/expanded variants |
| bibliographic-footer | Designed closure with nav, CTA, and repeated motif. | keyboard focus and reduced-motion behavior |
...
- Use Editorial Type surface rules instead of default card styling.
...
```tsx
export function EditorialTypeSection() {
  return (
      ...
      <div className="mx-auto grid w-full max-w-7xl grid-cols-1 gap-10 md:grid-cols-12 md:gap-6">
        <div className="md:col-span-5">
          ...
          <h2 className="text-balance text-4xl font-semibold leading-[0.95] tracking-tight md:text-7xl">
            A concrete promise with Editorial Type identity.
          </h2>
        </div>
...
Motion must be specific to Editorial Type. It should never be generic scroll-fade spam.
...
```css
[data-style-motion="editorial-type"] {
  transition:
...
[data-style-motion="editorial-type"]:is(:hover, :focus-visible, [data-active="true"]) {
...
- Hero or discovery area that makes the Editorial Type identity obvious.
...
Style-specific application: use poster-title-hero, issue-index-table, pullquote-panel, caption-grid as the first component candidates. Use the accent/radius/surface rules from the operating mode.
...
- Use density appropriate to Editorial Type: Medium. Text-rich with deliberate pacing.
...
- First viewport unmistakably expresses Editorial Type.
